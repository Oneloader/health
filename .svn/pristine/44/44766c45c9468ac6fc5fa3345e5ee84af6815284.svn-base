...
<script>
    ...
    <div class="tools auth-botton">
        ...
        <button url="<?php echo U('article/setStatus?status=1');?>" class="btn ajax-post" target-form="ids">审 核</button>
    </div>
    ...
    <tbody>
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a href="<?php echo U('Article/edit?cate_id='.$vo['category_id'].'&id='.$vo['id']);?>">编辑</a>
				<a href="<?php echo U('Article/setStatus?ids='.$vo['id'].'&status=1');?>" class="ajax-get">审核</a>
