...
<div class="panel panel-default">
    <div class="tab-wrap  panel-body ">	    
        ...
        <div class="tab-content">
            <form action="<?php echo U('save');?>" method="post" class="form col-lg-6" role="form" >
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?><div class="form-group">
                    ...
                    <?php case "2": ?><textarea name="config[<?php echo ($config["name"]); ?>]" class="form-control"><?php echo ($config["value"]); ?></textarea><?php break;?>
                    <?php case "3": ?><textarea name="config[<?php echo ($config["name"]); ?>]" class="form-control"><?php echo ($config["value"]); ?></textarea><?php break;?>
                ...
                <div class="form-group">
                    ...
                    <div class="controls">
                        <?php if(empty($list)): ?><button type="submit" disabled class="btn btn-default submit-btn disabled" target-form="form">确 定</button>
                        <?php else: ?>
                          <button type="submit" class="btn btn-default submit-btn ajax-post" target-form="form">确 定</button><?php endif; ?>				
