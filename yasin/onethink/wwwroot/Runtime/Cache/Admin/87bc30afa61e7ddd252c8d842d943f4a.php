        ...
        <form id="form" action="<?php echo U('update');?>" method="post" class="form " role="form" >
            <div class="tab-content panel-body">             
                ...
                <?php $_result=parse_config_attr($model['field_group']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><div id="tab<?php echo ($key); ?>" class="tab-pane fade <?php if(($key) == "1"): ?>in active<?php endif; ?> tab<?php echo ($key); ?>">
                    <?php if(is_array($fields[$key])): $i = 0; $__LIST__ = $fields[$key];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; if($field['is_show'] == 1 || $field['is_show'] == 3): ?><div class="form-group">
                            ...
                            <div class="form-group">
                                <?php switch($field["type"]): case "num": ?><input type="text" class="form-control" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"><?php break;?>
                                    <?php case "string": ?><input type="text" class="form-control" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"><?php break;?>
                                    <?php case "textarea": ?><textarea name="<?php echo ($field["name"]); ?>" class="form-control" ><?php echo ($data[$field['name']]); ?></textarea><?php break;?>
                                    <?php case "datetime": ?><input type="datetime" name="<?php echo ($field["name"]); ?>" class="form-control " value="<?php echo (time_format($data[$field['name']])); ?>" placeholder="请选择时间" /><?php break;?>
                                    ...
                                    <?php case "editor": ?><textarea name="<?php echo ($field["name"]); ?>" class="form-control" ><?php echo ($data[$field['name']]); ?></textarea>
                                        <?php echo hook('adminArticleEdit', array('name'=>$field['name'],'value'=>$data[$field['name']])); break;?>
                                    <?php case "picture": ?><div class="form-group">
                                    ...
                                    <?php default: ?>
                                    <input type="text" class="form-control" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"><?php endswitch;?>
                            </div>
                ...
                <div class="form-group">
                    ...
                    <?php if(C('OPEN_DRAFTBOX') and (ACTION_NAME == 'add' or $data['status'] == 3)): ?><button class="btn btn-default save-btn" url="<?php echo U('article/autoSave');?>" target-form="form" id="autoSave">
                        存草稿
                    </button><?php endif; ?>
