...
<head>
    <meta charset="UTF-8">
    <title><?php echo ($meta_title); ?>|Yasin管理平台</title>
    <link href="/Public/static/yasin/new_imgs/logo_icon.png" type="image/x-icon" rel="shortcut icon">
...
<body>
    ...
    <div class="header">
        <!-- Logo -->
        <span class="logo" style="margin-top: 7px;margin-right: 10px"><img src="/Public/static/yasin/new_imgs/logo.png"></span>
        <!-- /Logo -->
        ...
        <?php if(!empty($sub_menu)): ?><h3>
            ...
            <ul class="side-sub-menu <?php if(($sub_menu["current"]) != "1"): ?>subnav-off<?php endif; ?>">
                <?php if(is_array($sub_menu['_child'])): $i = 0; $__LIST__ = $sub_menu['_child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i;?><li <?php if($menu['id'] == $cate_id or $menu['current'] == 1): ?>class="current"<?php endif; ?>>
                        <?php if(($menu['allow_publish']) > "0"): ?><a class="item" href="<?php echo (U($menu["url"])); ?>"><i class="icon"></i><?php echo ($menu["title"]); ?></a><?php else: ?><a class="item" href="javascript:void(0);"><?php echo ($menu["title"]); ?></a><?php endif; ?>
...
<div class="tab-wrap">
	...
	<form id="form" action="<?php echo U('update');?>" method="post" class="form-horizontal">
            ...
            <?php if(is_array($fields[$key])): $i = 0; $__LIST__ = $fields[$key];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; if($field['is_show'] == 1 || $field['is_show'] == 3): ?><div class="form-item cf">
                    ...
                    <div class="controls">
                        <?php switch($field["type"]): case "num": ?><input type="text" class="text input-mid" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"><?php break;?>
                            ...
                            <?php case "sort": ?><input type="text" class="text input-mid" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"><?php break;?>

                            <?php case "hospital": ?><select name="<?php echo ($field["name"]); ?>">
                                    <?php $_result=parse_field_attr($field['extra']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php if(($data[$field['name']]) == $key): ?>selected<?php endif; ?>><?php echo ($vo); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                </select><?php break;?>
                            ...
                            <?php case "pictures": echo hook('UploadImages', array('name'=>$field['name'],'value'=>$data[$field['name']])); break;?>
                            <?php case "images": echo hook('UploadPic', array('name'=>$field['name'],'value'=>$data[$field['name']])); break;?>
                            <?php case "file": ?><div class="controls">
									<input type="file" id="upload_file_<?php echo ($field["name"]); ?>">
									<!--<input type="hidden" name="<?php echo ($field["name"]); ?>" value="<?php echo think_encrypt(json_encode(get_table_field($data[$field['id']],'id','','File')));?>"/>-->
        ...
        <div class="cont-ft">
            <div class="copyright">
                <div class="fl">欢迎使用<a href="#" target="_blank">Yasin</a>管理平台</div>
