...
//                                    $v['current']       =   true;
    ...
    protected function getDocumentList($cate_id=0,$model_id=null,$position=null,$field=true,$group_id=null){
        ...
        $list = $this->lists($Document,$map,'level ASC,DOCUMENT.id DESC',$field);
    ...
    public function add(){
        ...
        if ($model_id = 29){
            $cate = D('category')
                ...
                ->where(['id'=>$cate_id])
                ->find();
            $atherecate = D('category')
                ->field('id,title')
                ->where(['pid'=>$cate['pid']])
                ->find();

            $hospital = D('document_hospital_details')
                ->alias('h')
                ->field('h.id,h.list_title')
                ->join('one_document as d ON h.id=d.id')
                ->where(['d.category_id'=>$atherecate['id'],'d.status'=>1])
                ->select();

            foreach ($hospital as $ho){
                $hospitals .= implode(":",$ho)."\n";
            }
            $data = array('extra'=>$hospitals);
            D('attribute')->where(['id'=>169])->setField($data);
        }
...
//        var_dump($_POST);exit;
        if (!empty($_POST['sort'])){
            $_POST['level'] = $_POST['sort'];
        }
        if ($_POST['list_title']){
            $_POST['title'] = $_POST['list_title'];
        }
        $document   =   D('Document');
        $res = $document->update();
        if(!$res){
